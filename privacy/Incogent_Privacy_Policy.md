...
Each immutable batch contains its UTC sealing/reporting date, stable/prerelease channel, schema and consent-notice versions, and a random batch identifier used only to prevent duplicate counting. Its counters are grouped by their UTC activity dates and release channels. A report can contain increments from multiple dates, and later reports may add new increments to an earlier date. Retrying a batch does not add its counts again. Empty batches can report participation without any new Action or session activity. No account email, license identifier, or persistent installation identifier is included. Reporting app-days are approximate participation counts, including apps left open and idle, not unique people or proof of human interaction.
...
After explicit opt-in, Blackbird attempts its first report immediately and asynchronously. It becomes eligible to create a new batch on a later UTC day while already running, including when idle, or on its next launch/resume. It does not wait for an activity day to finish. Later observations accumulate for the next eligible batch. It creates at most one new batch per UTC day and does not drain multiple distinct batches on a single UTC day. Network failures may delay delivery. It does not launch Blackbird or a separate process, wake the computer, or make startup, Actions, or shutdown wait for a network response.

Each upload is limited to 2 KiB and ten date/channel counter buckets. There is at most one sealed pending batch plus bounded mutable counts, with at most 64 KiB of payload state in total. Mutable counts expire 30 days after midnight UTC on their activity date. A sealed batch expires as a whole 30 days after midnight UTC on its oldest included activity date, or its reporting date when empty. Retried or newly sealed data does not gain a fresh activity-retention window. Blackbird makes at most three attempts per UTC day, including retries, separated by at least one hour and any longer server-directed delay. Counts may be delayed or lost; complete histories are not collected.

Turning off reporting removes unsent counts and payloads. Small local non-payload safety controls remain to prevent restarts or consent toggles bypassing daily limits; they are not uploaded.
